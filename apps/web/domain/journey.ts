...
import { normaliseMultiSelectValue, questionnaireFieldIsVisible } from "./rules";
import type {
  ProblemBrief,
  QuestionnaireField,
  QuestionnaireSchema,
 ...
 *
 * A multi_select value additionally goes through normaliseMultiSelectValue
 * (domain/rules.ts): duplicates are silently collapsed and click-order is
 * canonicalised into the field's own declared option order, but a
 * contradictory value — the field's exclusiveValue (e.g. "unsure") present
 * alongside any other selection, such as a corrupted/hand-edited
 * `["unsure","leak","leak"]` — is DROPPED entirely rather than guessed at,
 * so the field reads back as unanswered and the owner is asked again. This
 * is the schema-aware restoration boundary the rest of the app (journey
 * state, generated brief, submission payload) relies on never seeing that
 * shape.
 */
function sanitiseResponses(
  ...
  for (const [fieldId, fieldValue] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (field.type === "multi_select") {
      const normalised = normaliseMultiSelectValue(field, fieldValue as string[]);
      if (!normalised) continue; // contradictory exclusive+real — drop, fail closed
      sanitised[fieldId] = normalised;
      continue;
    }
...
/**
 * Applies the same normaliseMultiSelectValue rule (domain/rules.ts) to a
 * STORED brief's own observedFacts arrays, not just the draft responses
 * sanitiseResponses already covers. A brief generated by the current
 * buildRepairBrief is already normalised, but a brief persisted by an
 * earlier deploy (before this normalisation existed), or a brief record
 * that was hand-edited, could still carry a contradictory or duplicated
 * array — and this exact object is what an already-in-progress journey's
 * resume path hands straight to submission as generated_brief (see
 * RepairSubmissionPanel's generatedBrief prop), with no further pass
 * through buildRepairBrief. A contradictory field (exclusiveValue alongside
 * a real selection) is dropped to undefined — the same fail-closed policy
 * as sanitiseResponses — rather than guessed at; a duplicated/reverse-order
 * array is deduplicated and reordered into the schema's own option order.
 * Historical scalar values pass through unchanged.
 */
function normaliseObservedFactsForCategory(
  category: RepairCategoryId,
  observedFacts: ProblemBrief["observedFacts"],
): ProblemBrief["observedFacts"] {
  if (!observedFacts) return observedFacts;
  const schema = questionnaireByCategory[category];
  const fieldsById = new Map(allFieldsOf(schema).map((field) => [field.id, field]));
  const normalised = { ...observedFacts };
  for (const key of OBSERVED_FACTS_ARRAY_CAPABLE_KEYS as Set<"branchFirst" | "branchSecond" | "branchThird">) {
    const raw = normalised[key];
    if (!Array.isArray(raw)) continue;
    const field = fieldsById.get(key);
    const result = field ? normaliseMultiSelectValue(field, raw) : null;
    normalised[key] = result && result.length > 0 ? result : undefined;
  }
  return normalised;
}
...
export function readJourneyBrief(
  ...
  try {
    ...
    const brief: ProblemBrief = {
      ...parsed.brief,
      observedFacts: normaliseObservedFactsForCategory(category, parsed.brief.observedFacts),
    };
    return {
      journeyId,
      category,
