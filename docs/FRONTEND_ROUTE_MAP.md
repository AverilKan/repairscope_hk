...
| `/landlord/repairs/:repairId/brief` | Review and correct a neutral brief, then submit for manual review | loading, ready, correcting, corrected, correction failed, contact form, submitting, submitted with reference | `ContractorBriefService.getForRepair/applyCorrection`, `RepairSubmissionService.submit` | fully public — no account; RepairSubmission created for manual founder review | `ceilingBrief` through mock service |
...
| `/operator` | Real submission review (list, detail, status/notes) | loading, access denied, list, detail, saving, save failed | `OperatorSubmissionService.list/get/updateStatus` (`GET`/`PATCH /api/repair-submissions`) | Clerk session + operator capability, gated by `OperatorGate` (UX) and `AuthorizationService.require_operator` (real) | `MockOperatorSubmissionService` fixture |
